  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent
        ...
        style={{
          borderRadius:
            mode === "view" ? "0.5rem 0.5rem 1.5rem 1.5rem" : "0.5rem",
        }}
      >
        {mode === "view" ? (
          <div className="relative w-full h-full bg-[#63A053] dark:bg-[#4a7a3d] overflow-hidden flex flex-col rounded-b-3xl rounded-t-lg shadow-xl">
            ...
            <div className="shrink-0 flex flex-col items-center text-white pt-12 pb-10">
              {/* Title & Subtitle */}
              <div className="text-center mb-10">
                <h3 className="text-xl font-semibold mb-1.5">{title}</h3>
                {subtitle && (
                  <div className="text-sm opacity-95">{subtitle}</div>
                )}
              </div>
              ...
              <div className="flex items-center gap-4">
                {/* Calendar Icon */}
                <div className="w-12.25 h-11.75 flex items-center justify-center shrink-0">
                  <svg
            ...
            {/* White Content Card */}
            <div className="flex-1 relative bg-white dark:bg-gray-900 rounded-b-3xl rounded-t-4xl overflow-visible">
              {/* Participants Section */}
              <div className="pt-4 px-8 pb-20">
                {/* Participants Label */}
                <div className="flex items-center justify-center gap-2 mb-4">
                  <span className="text-lg text-[#343E47]/50 dark:text-gray-400">
                    Participants
                  </span>
                  <span className="bg-[#F5C00FE8] dark:bg-yellow-600 text-white text-lg font-medium rounded-full px-2.5 py-0.5 min-w-6 text-center">
                    {participants.length}
                  </span>
                </div>
                ...
                <div className="flex gap-3 items-center justify-center mb-5">
                  {participants.map((p) => (
                    <div
                      key={p.id}
                      className="w-12 h-12 rounded-full border-[3px] border-white bg-[#D9D9D9] bg-cover bg-center shrink-0"
                      style={{
                  ...
                  <button
                    ...
                    className="w-12 h-12 rounded-full border-[3px] border-white dark:border-gray-800 bg-[#EEF6EE] dark:bg-gray-700 text-[#63A053] dark:text-green-400 flex items-center justify-center text-3xl font-light cursor-pointer hover:bg-[#E0F0E0] dark:hover:bg-gray-600 transition-colors shrink-0"
                  >
                    +
                  </button>
                </div>
                ...
                {/* Description */}
                <div className="text-sm text-[#333333] dark:text-gray-300 leading-relaxed text-left">
                  {description ||
                    "Gather and validate all legal files required for the design drafts, or updated business registration, and operating licenses. Ensure the team is fully ready before moving to the next stage. Gather and validate all legal files required for the design drafts, or updated business registration, and operating licenses. Ensure the team is fully ready before moving to the next stage."}
                </div>
              </div>
          ...
          <div className="flex flex-col h-full bg-white dark:bg-gray-900 overflow-hidden rounded-lg shadow-xl">
            <div className="w-full h-20 flex items-center justify-center bg-[#63A053] dark:bg-[#4a7a3d] relative shrink-0">
              ...
              <button
                ...
                className="absolute right-4 text-white hover:bg-white/20 dark:hover:bg-white/10 p-2 rounded-full transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
